...
(define (DrawPiezes positions counter turn jaqueState lPieze lIndex)
    ...
    (if (< counter 63)
        (DrawPiezes positions (+ counter 1) turn (DetectJaque positions piezeG counter turn jaqueState) lPieze lIndex)
        (void)
    )
    (if (= jaqueState 1)
        (if (VerificateJaqueMate positions 1 1 turn)
...
(define (VerificateJaqueMate positions counter step turn)
    (define (SearchKing search count)
        (if (< count (string-length positions))
            (if (equal? (string-ref positions count) search)
                count
