...
(define (DrawPiezes positions counter turn jaqueState lPieze lIndex)
    ...
    (if (< counter 63)
        (if (= jaqueState 1)
            (DrawPiezes positions (+ counter 1) turn jaqueState lPieze (JaquePieze lIndex counter))
            (DrawPiezes positions (+ counter 1) turn (DetectJaque positions piezeG counter turn jaqueState) lPieze counter)
        )
        (if (= jaqueState 1)
            (if (VerificateJaqueMate positions 1 1 turn lIndex)
                (begin
                    ((draw-pixmap board) "PiezasAjedrez/JaqueMate.png" (make-posn 565 290))
                    (GetPosition positions 0 #\L 0 0 turn 0)
                )
                (begin
                            ...
                            (if (equal? key #\Z)
                                ...
                                (if (equal? key #\x)
                                    ...
                                    (if (equal? key #\X)
                                        (string-append (substring positions 0 selectPiezeIndex) "R" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions)))
                                        (if (equal? key #\T)
                                            (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "T" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                            (if (equal? key #\R)
                                                (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "R" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                                (if (equal? key #\r)
                                                    (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "r" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                                    (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "t" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                                )
                                            )
                                        )
                                    )
                            ...
                            (if (equal? key #\Z)
                                ...
                                (if (equal? key #\x)
                                    ...
                                    (if (equal? key #\X)
                                        (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "R" (substring positions (+ selectPiezeIndex 1) (string-length positions)))
                                        (if (equal? key #\T)
                                            (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "T" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                            (if (equal? key #\R)
                                                (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "R" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                                (if (equal? key #\r)
                                                    (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "r" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                                    (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "t" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                                )
                                            )
                                        )
                                    )
                                ...
                                (if (equal? key #\Z)
                                    ...
                                    (if (equal? key #\x)
                                        ...
                                        (if (equal? key #\X)
                                            (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "R" (substring positions (+ selectPiezeIndex 1) (string-length positions)))
                                            (if (equal? key "A")
                                                (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "A" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                                (if (equal? key "a")
                                                    (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "a" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                                    (if (equal? key "R")
                                                        (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "R" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                                        (if (equal? key "r")
                                                            (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "r" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                ...
                                (if (equal? key #\Z)
                                    ...
                                    (if (equal? key #\x)
                                        ...
                                        (if (equal? key #\X)
                                            (string-append (substring positions 0 selectPiezeIndex) "R" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions)))
                                            (if (equal? key "A")
                                                (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "A" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                                (if (equal? key "a")
                                                    (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "a" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                                    (if (equal? key "R")
                                                        (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "R" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                                        (if (equal? key "r")
                                                            (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "r" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
    ...
    (if (or (equal? key #\P) (equal? key #\V))
            ...
            (if (and (> preIndex 7) (< preIndex 16) (= selectPiezeIndex (+ preIndex 16)))
                (if (equal? (string-ref positions selectPiezeIndex) #\-)
                    (if (equal? (string-ref positions (- selectPiezeIndex 8)) #\-)
                        (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "P" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                        (if (equal? key #\V)
                            #f
                            (PlayGame positions #\L 0 #\L 0 0 0 0 0 turn jaqueState)
                        )
                    ...
                    (if (equal? (string-ref positions selectPiezeIndex) #\-)
                        (begin
                            (if (and (= posY 7) (= preY 6))
                                (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) (Coronation key) (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "P" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                            )
                        )
                        (if (equal? key #\V)
        ...
        (begin
            (if (and (> preIndex 46) (< preIndex 56) (= selectPiezeIndex (- preIndex 16)))
                (if (equal? (string-ref positions selectPiezeIndex) #\-)
                    (if (equal? (string-ref positions (+ selectPiezeIndex 8)) #\-)
                        (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "p" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                        (if (equal? key #\v)
                            #f
                            (PlayGame positions #\L 0 #\L 0 0 0 0 0 turn jaqueState)
                        )
                ...
                (if (= selectPiezeIndex (- preIndex 8))
                    (if (equal? (string-ref positions selectPiezeIndex) #\-)
                        (if (and (= posY 0) (= preY 1))
                            (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) (Coronation key) (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                            (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "p" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                        )
                        (if (equal? key #\v)
                            #f
                        ...
                        (if (equal? key #\v)
                            ...
                            (if (equal? key #\z)
                                ...
                                (if (equal? key #\Z)
                                    (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "C" (substring positions (+ selectPiezeIndex 1) (string-length positions)))
                                    (if (equal? key "C")
                                        (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "C" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                        (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "c" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                    )
                                )
                            )
                        )
                        ...
                        (if (equal? key #\v)
                            ...
                            (if (equal? key #\z)
                                ...
                                (if (equal? key #\Z)
                                    (string-append (substring positions 0 selectPiezeIndex) "C" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions)))
                                    (if (equal? key "C")
                                        (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "C" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                        (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "c" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                    )
                                )
                            )
                        )
                    ...
                    (if (= 1 (- posX preX))
                        (if (equal? key #\z)
                            ...
                            (if (equal? key #\Z)
                                (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "Y" (substring positions (+ selectPiezeIndex 1) (string-length positions)))
                                (if (equal? key "Y")
                                    (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "Y" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                    (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "y" (substring positions (+ selectPiezeIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                )
                            )
                        )
                        (if (or (equal? key #\z) (equal? key #\Z))
                    ...
                    (if (= 1 (- preX posX))
                        (if (equal? key #\z)
                            ...
                            (if (equal? key #\Z)
                                (string-append (substring positions 0 selectPiezeIndex) "Y" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions)))
                                (if (equal? key "Y")
                                    (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "Y" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 0 jaqueState key selectPiezeIndex)
                                    (DrawPiezes (string-append (substring positions 0 selectPiezeIndex) "y" (substring positions (+ 1 selectPiezeIndex) preIndex) "-" (substring positions (+ preIndex 1) (string-length positions))) 0 1 jaqueState key selectPiezeIndex)
                                )
                            )
                        )
                        (if (or (equal? key #\z) (equal? key #\Z))
...
(define (DetectJaque positions piezeV indexV turn jaqueState)
    ...
    (define blackPiezes "-TCARYP")
    (define whitePiezes "-tcaryp")
    (displayln "Entró a VerificateJaque")
    (if (= jaqueState 0)
        (if (equal? (~a piezeV) "t")
...
(define (VerificateJaqueMate positions counter step turn jaqueIndex)
    (define (SearchKing search count)
        (if (< count (string-length positions))
            (if (equal? (string-ref positions count) search)
                count
    ...
    (define (TestPos value counte)
        (displayln "Entró a TestPos")
        (if (< counte (string-length value))
            (begin
                ;(printf "--~a=~a\n" (string-ref value counte) counte)
    ...
    (define (ComparateStrings posE)
        (displayln "Entró a ComparateStrings:")
        (printf "~a-------\n" posE)
    ...
    (if (< counter (string-length positions))
        (if (= step 1)
            (if (= turn 0)
                (if (ComparateStrings (MoveKing positions (- posKingW counter) posKingW #\z (remainder posKingW 8) (remainder (- posKingW counter) 8) (quotient posKingW 8) (quotient (- posKingW counter) 8) turn blackPiezes 0))
                    #f
                    (if (and (< posKingW 64) (> posKingW 54) (= counter 1))
                        (if (ComparateStrings (MoveKing positions (+ posKingW counter) posKingW #\z (remainder posKingW 8) (remainder (+ posKingW counter) 8) (quotient posKingW 8) (quotient (+ posKingW counter) 8) turn blackPiezes 0))
                            #f
                            (if (= counter 1)
                                (VerificateJaqueMate positions (+ counter 7) step turn jaqueIndex)
                                (if (< counter 10)
                                    (VerificateJaqueMate positions (+ counter 1) step turn jaqueIndex)
                                    (VerificateJaqueMate positions 0 2 turn jaqueIndex)
                                )
                            )
                        )
                        (if (< posKingW 54)
                ...
                (if (ComparateStrings (MoveKing positions (+ posKingB counter) posKingB #\Z (remainder posKingB 8) (remainder (+ posKingB counter) 8) (quotient posKingB 8) (quotient (+ posKingB counter) 8) turn whitePiezes 0))
                    #f
                    (if (and (< posKingW 8) (>= posKingW 0) (= counter 1))
                        (if (ComparateStrings (MoveKing positions (- posKingB counter) posKingB #\Z (remainder posKingB 8) (remainder (- posKingB counter) 8) (quotient posKingB 8) (quotient (- posKingB counter) 8) turn whitePiezes 0))
                            #f
                            (if (= counter 1)
                                (VerificateJaqueMate positions (+ counter 7) step turn jaqueIndex)
            ...
            (begin
                ;(printf "~a, ~a, ~a, ~a\n" step turn counter (~a (string-ref positions counter)))
                (if (= step 2)
                    (if (= turn 0)
                        (if (< counter (string-length positions))
                            (if (equal? (~a (string-ref positions counter)) "t")
                                (if (ComparateStrings (MoveTorreAndQueen positions jaqueIndex counter #\z (remainder counter 8) (remainder jaqueIndex 8) (quotient counter 8) (quotient jaqueIndex 8) turn blackPiezes 0))
                                    #f
                                    (VerificateJaqueMate positions (+ counter 1) step turn jaqueIndex)
                                )
                                (if (equal? (~a (string-ref positions counter)) "a")
                                    (if (ComparateStrings (MoveAlfilAndQueen positions jaqueIndex counter #\z (remainder counter 8) (remainder jaqueIndex 8) (quotient counter 8) (quotient jaqueIndex 8) turn blackPiezes 0))
                                        #f
                                        (VerificateJaqueMate positions (+ counter 1) step turn jaqueIndex)
                                    )
                                    (if (equal? (~a (string-ref positions counter)) "r")
                                        (if (ComparateStrings (MoveTorreAndQueen positions jaqueIndex counter #\x (remainder counter 8) (remainder jaqueIndex 8) (quotient counter 8) (quotient jaqueIndex 8) turn blackPiezes 0))
                                            #f
                                            (if (ComparateStrings (MoveAlfilAndQueen positions jaqueIndex counter #\x (remainder counter 8) (remainder jaqueIndex 8) (quotient counter 8) (quotient jaqueIndex 8) turn blackPiezes 0))
                                                #f
                                                (VerificateJaqueMate positions (+ counter 1) step turn jaqueIndex)
                                            )
                                        )
                                        (if (equal? (~a (string-ref positions counter)) "y")
                                            (if (ComparateStrings (MoveKing positions jaqueIndex counter #\z (remainder counter 8) (remainder jaqueIndex 8) (quotient counter 8) (quotient jaqueIndex 8) turn blackPiezes 0))
                                                #f
                                                (VerificateJaqueMate positions (+ counter 1) step turn jaqueIndex)
                                            )
                                            (if (equal? (~a (string-ref positions counter)) "c")
                                                (if (ComparateStrings (MoveHorse positions jaqueIndex counter #\z (remainder counter 8) (remainder jaqueIndex 8) (quotient counter 8) (quotient jaqueIndex 8) turn blackPiezes 0))
                                                    #f
                                                    (VerificateJaqueMate positions (+ counter 1) step turn jaqueIndex)
                                                )
                                                (if (equal? (~a (string-ref positions counter)) "p")
                                                    (if (ComparateStrings (MovePeon positions jaqueIndex counter #\z (remainder counter 8) (remainder jaqueIndex 8) (quotient counter 8) (quotient jaqueIndex 8) turn blackPiezes 0))
                                                        #f
                                                        (VerificateJaqueMate positions (+ counter 1) step turn jaqueIndex)
                                                    )
                                                    (VerificateJaqueMate positions (+ counter 1) step turn jaqueIndex)
...
;(VerificateJaqueMate "TCA-YACTPPPP-PPP----P------------------R-----p--ppppp-p-tcaryact" 59 2 0 39)