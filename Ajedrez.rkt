...
(define (DrawPiezes positions counter turn jaqueState lPieze lIndex)
    ...
    (if (< counter 63)
        (if (= jaqueState 1)
            (DrawPiezes positions (+ counter 1) turn (DetectJaque positions piezeG counter turn jaqueState) lPieze (JaquePieze lIndex counter))
                ...
                (if (= 2 (- posY preY))
                    (if (= (- preX 1) posX)
                        (if (equal? key #\v)
                            #t
                            (if (equal? key #\z)
                                (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "c" (substring positions (+ selectPiezeIndex 1) (string-length positions)))
                                (if (equal? key #\Z)
                                    (DrawPiezes (string-append (substring positions 0 preIndex) "-" (substring positions (+ 1 preIndex) selectPiezeIndex) "C" (substring positions (+ selectPiezeIndex 1) (string-length positions)))
                                    (if (equal? key "C")
            ...
            (if (equal? (~a piezeV) "r")
                ...
                (if (equal? (~a piezeV) "a")
                    ...
                    (if (equal? (~a piezeV) "c")
                        ...
                        (if (equal? (~a piezeV) "p")
                            (if (MovePeon positions posKingB indexV #\v (remainder indexV 8) (remainder posKingB 8) (quotient indexV 8) (quotient posKingB 8) turn whitePiezes 0)
                                (+ 0 1)
                                (+ 0 0)
                            )
                            (if (equal? (~a piezeV) "T")
...
(define (VerificateJaqueMate positions counter step turn jaqueIndex)
    ...
    (define blackPiezes "-TCARYP")
    (define whitePiezes "-tcaryp")
    (define piezeM (~a (string-ref positions counter)))
    (if (= step 1)
        (if (= turn 0)
            (if (TestPos (MoveKing positions (- posKingW counter) posKingW #\z (remainder posKingW 8) (remainder (- posKingW counter) 8) (quotient posKingW 8) (quotient (- posKingW counter) 8) turn blackPiezes 0) 0)
                #f
            ...
            (if (TestPos (MoveKing positions (+ posKingB counter) posKingB #\Z (remainder posKingB 8) (remainder (+ posKingB counter) 8) (quotient posKingB 8) (quotient (+ posKingB counter) 8) turn whitePiezes 0) 0)
                #f
